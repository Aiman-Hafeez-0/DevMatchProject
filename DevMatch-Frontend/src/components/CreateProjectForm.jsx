...
export default function CreateProjectForm() {
  ...
  const addTech = (tech) => {
    ...
    if (!t) return;
    if (
      formData.tech_stack.map((x) => x.toLowerCase()).includes(t.toLowerCase())
    )
      return;
  ...
  const removeTech = (tech) => {
    setFormData((p) => ({
      ...p,
      tech_stack: p.tech_stack.filter((t) => t !== tech),
    }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.tech_stack.length === 0) {
      toast.warning("Add at least one technology");
      return;
    }
              ...
              {[
                {
                  name: "is_paid",
                  label: "💰 Paid Project",
                  desc: "Collaborators will be compensated",
                },
                {
                  name: "is_private",
                ...
                >
                  <input
                    ...
                    onChange={handleChange}
                    style={{ marginTop: 2, accentColor: "#2952A3" }}
                  />
                  <div>
                    <div
                      style={{

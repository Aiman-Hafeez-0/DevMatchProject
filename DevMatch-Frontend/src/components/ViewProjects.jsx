...
export default function ViewProjects() {
  ...
  const fetchPendingRequests = () => {
    ...
    fetch("http://localhost:5000/projects/pending-requests", {
      credentials: "include",
    })
      .then((r) => (r.ok ? r.json() : []))
  ...
  const handleAcceptRequest = async (projectId, userId) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:5000/projects/${projectId}/accept/${userId}`,
        {
          method: "PUT",
          credentials: "include",
        },
  ...
  const handleRejectRequest = async (projectId, userId) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:5000/projects/${projectId}/reject/${userId}`,
        {
          method: "DELETE",
          credentials: "include",
        },
  ...
  const sendInvite = async () => {
    ...
    try {
      const res = await fetch(
        `http://localhost:5000/projects/${invitingProject.id}/invite`,
        {
          method: "POST",
  ...
  useEffect(() => {
    Promise.all([
      fetch("http://localhost:5000/projects/my-projects", {
        credentials: "include",
      }).then((r) => (r.ok ? r.json() : [])),
      fetch("http://localhost:5000/projects/joined-projects", {
        credentials: "include",
      }).then((r) => (r.ok ? r.json() : [])),
    ])
      .then(([owned, joined]) => {
  ...
  const addProblemStatement = () => {
    ...
    setEditForm((prev) => ({
      ...prev,
      problem_statements: [...(prev.problem_statements || []), s],
    }));
    setNewStatement("");
  };
        ...
        setProjects((prev) =>
          prev.map((p) =>
            p.id === editingProject.id
              ? { ...p, ...payload, tech_stack: payload.tech_stack }
              : p,
          ),
        );
    ...
    <div style={BG}>
      <div style={{ maxWidth: 860, margin: "0 auto" }}>
        ...
        {activeTab === "owned" && projects.length === 0 && (
          ...
          >
            ...
            <p style={{ color: "#8A9BBC", marginBottom: 16 }}>
                ...
                >
                  ...
                  {techStack.length > 0 && (
                    <div className="d-flex flex-wrap gap-1 mb-3">
                      ...
                      {techStack.length > 6 && (
                        <span style={{ color: "#8A9BBC", fontSize: "0.72rem" }}>
                  ...
                  <div
                    ...
                    style={{ borderTop: "1px solid #D0DAEE", paddingTop: 12 }}
                  >
                    {project.github_repo && (
                      <a
                    ...
                    <button
                      ...
                      style={{
                        ...
                        color: "#2952A3",
                        border: project.is_private
                          ? "1.5px solid #2952A3"
                          : "1px solid #C7D3EE",
                        borderRadius: 8,
                        fontSize: "0.78rem",
                        fontWeight: project.is_private ? 700 : 500,
                      }}
                      onClick={() => openInvite(project)}
                      title={
                        project.is_private
                          ? "Invite someone — required for private projects"
                  ...
                  <div className="d-flex justify-content-between align-items-start gap-2 mb-2">
                    <div>
                      ...
                      <small style={{ color: "#8A9BBC" }}>
                        by {project.owner_name} · Joined{" "}
                        {new Date(project.joined_at).toLocaleDateString([], {
                          month: "short",
                          day: "numeric",
                          year: "numeric",
                        })}
        ...
        {activeTab === "requests" &&
          ...
          pendingRequests.length === 0 && (
            ...
            >
              <div style={{ fontSize: "2.5rem", marginBottom: 12 }}>📭</div>
              <p style={{ color: "#7A8BB0", marginBottom: 0 }}>
                No pending requests right now.
              </p>
              <p
                style={{ color: "#A6B3CF", fontSize: "0.82rem", marginTop: 4 }}
              >
                When someone requests to join one of your projects, it'll show
                up here.
              </p>
            </div>
          )}
          ...
          pendingRequests.length > 0 && (
            <div className="d-flex flex-column gap-3">
              {pendingRequests.map((req) => {
                const isProcessing =
                  processingRequestId === `${req.project_id}-${req.user_id}`;
                    ...
                    <div className="d-flex justify-content-between align-items-start gap-3 flex-wrap">
                      ...
                      <div className="d-flex gap-2 flex-shrink-0">
                        <button
                          ...
                          onClick={() =>
                            handleAcceptRequest(req.project_id, req.user_id)
                          }
                        ...
                        <button
                          disabled={isProcessing}
                          onClick={() =>
                            handleRejectRequest(req.project_id, req.user_id)
                          }
                          className="btn btn-sm"
                          style={{
                            background: "#fff",
        ...
        >
          ...
          >
            ...
            <div className="mb-3">
              ...
              >
                Tech Stack{" "}
                <span style={{ fontWeight: 400, color: "#8A9BBC" }}>
                  (comma-separated)
                </span>
              </label>
              <input
            ...
            <div className="mb-3">
              ...
              >
                Remove one once a member has solved it — it'll no longer show as
                open work.
              </p>
            ...
            <div className="d-flex gap-4 mb-4">
              {[
                { name: "is_paid", label: "Paid Project" },
                { name: "is_private", label: "Private Project" },
              ].map(({ name, label }) => (
                <label
                  key={name}
                  style={{
      ...
      {invitingProject && (
        <div
          ...
          onClick={(e) => {
            if (e.target === e.currentTarget) setInvitingProject(null);
          }}
        >
          <div
            style={{
          ...
          >
            ...
            {invitingProject.is_private && (
              ...
              >
                🔒 This project is private — invited users are the only way for
                anyone to join it.
              </div>
            )}

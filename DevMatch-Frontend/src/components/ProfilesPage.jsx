...
export default function ProfilesPage() {
  ...
  useEffect(() => {
    fetch(`${API_URL}/profiles/all`, { credentials: "include" })
      .then((r) => r.json())
      .then((data) => {
  ...
  useEffect(() => {
    ...
    if (expFilter !== "All") result = result.filter((p) => p.experience_level === expFilter);
    setFiltered(result);
    ...
    <div style={BG}>
      <div style={{ maxWidth: 960, margin: "0 auto" }}>
        ...
        <div style={{ marginBottom: 24 }}>
          <h3 style={{ fontWeight: 700, color: "#1E2B4A", marginBottom: 4 }}>Developer Profiles</h3>
        ...
        <div
          ...
          style={{ background: "#fff", padding: "14px 16px", borderRadius: 12, boxShadow: "0 2px 10px rgba(0,0,0,0.06)" }}
        >
          <div style={{ flex: 1, minWidth: 200, position: "relative" }}>
            <FaSearch style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", color: "#ccc" }} />
            <input
              type="text"
              ...
              style={{
                ...
                border: "1.5px solid #D0DAEE", borderRadius: 8,
          ...
          <select
            ...
            style={{ padding: "8px 12px", border: "1.5px solid #D0DAEE", borderRadius: 8, fontSize: "0.85rem", outline: "none" }}
          >
            <option>All</option>
        ...
        <div className="row g-3">
          {filtered.map((profile) => {
            const expStyle = EXP_STYLE[profile.experience_level] || { background: "#F2F5FC", color: "#8A9BBC" };
                ...
                >
                  ...
                  <div className="d-flex align-items-center gap-3 mb-3">
                    ...
                    <div style={{ minWidth: 0 }}>
                      <div style={{ fontWeight: 700, color: "#1E2B4A", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                        {fullName || profile.user_name}
                      </div>
                      <div style={{ fontSize: "0.75rem", color: "#8A9BBC" }}>@{profile.user_name}</div>
                    </div>
                    {profile.experience_level && (
                      <span style={{ ...expStyle, borderRadius: 20, padding: "3px 10px", fontSize: "0.68rem", fontWeight: 600, marginLeft: "auto", flexShrink: 0 }}>
                        {profile.experience_level}
                      </span>
                    )}
                  </div>
                  ...
                  {profile.bio && (
                    <p style={{
                      color: "#666", fontSize: "0.82rem", lineHeight: 1.5, marginBottom: 12,
                    ...
                    <div className="d-flex flex-wrap gap-1 mb-3">
                      {skills.slice(0, 5).map((s) => (
                        <span key={s} style={{
                          ...
                          borderRadius: 6, padding: "2px 8px", fontSize: "0.68rem", fontWeight: 500,
                        }}>
                          {s}
                        </span>
                      ))}
                      {skills.length > 5 && (
                        <span style={{ color: "#8A9BBC", fontSize: "0.68rem", padding: "2px 4px" }}>+{skills.length - 5}</span>
                      )}
                    </div>
                  )}

                  {/* Footer */}
                  <div className="d-flex justify-content-between align-items-center mt-auto pt-2" style={{ borderTop: "1px solid #D0DAEE" }}>
                    <small style={{ color: "#8A9BBC" }}>
                      {profile.availability ? `${profile.availability} hrs/week` : ""}
                    </small>
                    <div className="d-flex gap-2 align-items-center" onClick={(e) => e.stopPropagation()}>
                      {profile.github_url && (
                        <a href={profile.github_url} target="_blank" rel="noreferrer" style={{ color: "#333", fontSize: "1.1rem" }}>
                          <FaGithub />
                        </a>
                      )}
                      <button

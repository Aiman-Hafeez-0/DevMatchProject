...
export default function SeekerProfileForm() {
  ...
  const [formData, setFormData] = useState({
    ...
    skills: [],
    experience_level: "", github_url: "", availability: "", bio: "",
  });

  useEffect(() => {
    fetch(`${API_URL}/profiles/profile`, { credentials: "include" })
      ...
      .then((data) => {
        ...
        if (Array.isArray(rawSkills)) skills = rawSkills;
          ...
          skills = rawSkills.replace(/[{}"]/g,"").split(",").map(s => s.trim()).filter(Boolean);
  ...
  const addSkill = (skill) => {
    const s = skill.trim();
    if (!s) return;
    if (formData.skills.map(x => x.toLowerCase()).includes(s.toLowerCase())) return;
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      const res = await fetch(`${API_URL}/profiles/profile`, {
        method: "PUT",
  ...
  const labelStyle = { fontSize: "0.82rem", fontWeight: 600, color: "#4A5B7A", marginBottom: 5, display: "block" };
      ...
      <div style={{ maxWidth: 580, margin: "0 auto" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          ...
          <p style={{ color: "#8A9BBC", fontSize: "0.85rem" }}>
            {isEditing ? "Keep your info up to date" : "Set up your profile to start matching with projects"}
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          {/* Personal Info */}
          <div style={{ background: "#fff", borderRadius: 14, padding: "24px 22px", marginBottom: 16, boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}>
            <h6 style={{ fontWeight: 700, color: "#2952A3", marginBottom: 16, textTransform: "uppercase", fontSize: "0.72rem", letterSpacing: 0.8 }}>
              Personal Information
            </h6>

            <div className="mb-3">
              <label style={labelStyle}>Username</label>
              <input style={inputStyle} name="username" value={formData.username} onChange={handleChange} onFocus={focusStyle} onBlur={blurStyle} placeholder="yourhandle" required />
            </div>

            <div className="row g-3 mb-3">
              <div className="col-6">
                <label style={labelStyle}>First Name</label>
                <input style={inputStyle} name="first_name" value={formData.first_name} onChange={handleChange} onFocus={focusStyle} onBlur={blurStyle} placeholder="Aiman" required />
              </div>
              <div className="col-6">
                <label style={labelStyle}>Last Name</label>
                <input style={inputStyle} name="last_name" value={formData.last_name} onChange={handleChange} onFocus={focusStyle} onBlur={blurStyle} placeholder="Siddiqui" required />
              </div>
            </div>

            <div className="mb-0">
              <label style={labelStyle}>Email</label>
              <input type="email" style={inputStyle} name="email" value={formData.email} onChange={handleChange} onFocus={focusStyle} onBlur={blurStyle} placeholder="you@example.com" required />
            </div>
          </div>
          ...
          <div style={{ background: "#fff", borderRadius: 14, padding: "24px 22px", marginBottom: 16, boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}>
            ...
            <div className="mb-3">
              ...
              <div style={{
                ...
                border: "1.5px solid #D0DAEE", borderRadius: 10, minHeight: 48,
                background: "#F2F5FC", marginBottom: 8,
              }}>
                ...
                <input
                  ...
                  style={{
                    ...
                    fontSize: "0.85rem", minWidth: 140, flex: 1,
                  }}
                />
              </div>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
                {SUGGESTED_SKILLS.filter(s => !formData.skills.map(x => x.toLowerCase()).includes(s.toLowerCase())).slice(0, 12).map((s) => (
                  <button
                    key={s} type="button"
                    onClick={() => addSkill(s)}
                    style={{
                      background: "#F2F5FC", color: "#8A9BBC",
            ...
            <div className="mb-0">
              ...
              <select
                ...
                onChange={handleChange} onFocus={focusStyle} onBlur={blurStyle} required
              >
          ...
          <div style={{ background: "#fff", borderRadius: 14, padding: "24px 22px", marginBottom: 16, boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}>
            ...
            <div className="mb-3">
              <label style={labelStyle}><FaGithub style={{ marginRight: 5 }} />GitHub URL</label>
              <input type="url" style={inputStyle} name="github_url" value={formData.github_url}
            ...
            <div className="mb-0">
              ...
              <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                <input
                  ...
                  placeholder="e.g. 10"
                />
                <span style={{ color: "#8A9BBC", fontSize: "0.85rem", whiteSpace: "nowrap" }}>hrs / week</span>
              </div>
            </div>
          </div>

          {/* Bio */}
          <div style={{ background: "#fff", borderRadius: 14, padding: "24px 22px", marginBottom: 24, boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}>
            <h6 style={{ fontWeight: 700, color: "#2952A3", marginBottom: 16, textTransform: "uppercase", fontSize: "0.72rem", letterSpacing: 0.8 }}>
              About You
            </h6>
            <textarea

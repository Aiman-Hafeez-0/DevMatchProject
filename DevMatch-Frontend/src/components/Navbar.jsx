...
export default function AppNavbar() {
  ...
  useEffect(() => {
    ...
    const fetchCount = () => {
      fetch("http://localhost:5000/projects/notifications", {
        credentials: "include",
      })
        .then((r) => (r.ok ? r.json() : []))
        .then((data) => {
  ...
  useEffect(() => {
    ...
    const fetchUnread = () => {
      fetch("http://localhost:5000/chat/unread-count", {
        credentials: "include",
      })
        .then((r) => (r.ok ? r.json() : null))
        .then((data) => {
          if (data) setUnreadChats(data.unread_rooms || 0);
        })
        .catch(() => {});
    };
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("devmatch:open-notifications", openHandler);
  }, []);
  ...
  const avatarLetter = (user?.user_name || user?.username || "?")
    .charAt(0)
    .toUpperCase();
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <span
            style={{ fontWeight: 800, fontSize: "1.25rem", color: "#1E2B4A" }}
          >
              ...
              <Link
                ...
                onMouseEnter={(e) => {
                  if (!isActive(to)) {
                    ...
                    e.currentTarget.style.color = "#2952A3";
                  }
                }}
                onMouseLeave={(e) => {
                  if (!isActive(to)) {
                    e.currentTarget.style.background = "transparent";
                    e.currentTarget.style.color = "#5E6E9A";
                  }
                }}
              >
                {label}
          ...
          {user ? (
            <>
              ...
              <Link
                ...
                onMouseEnter={(e) =>
                  (e.currentTarget.style.background = "#1E3D7A")
                }
                onMouseLeave={(e) =>
                ...
                {showUserMenu && (
                  ...
                  >
                    ...
                    >
                      ...
                      <div style={{ color: "#8A9BBC", fontSize: "0.75rem" }}>
                        @{user.user_name}
                      </div>
                    </div>

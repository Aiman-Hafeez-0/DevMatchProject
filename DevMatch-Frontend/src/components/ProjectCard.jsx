...
export default function ProjectCard({ project }) {
  ...
  const [requestStatus, setRequestStatus] = useState(
    project.participant_status || null,
  );
  ...
  const handleRequest = async (e) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:5000/projects/${project.id}/request`,
        {
          method: "POST",
  ...
  return (
    <div
      ...
      onMouseEnter={(e) =>
        (e.currentTarget.style.boxShadow = "0 6px 24px rgba(41,82,163,0.13)")
      }
      onMouseLeave={(e) =>
    ...
    >
      <div className="d-flex justify-content-between align-items-start gap-2 mb-2">
        {/* Title + owner */}
        <div>
          <h5 style={{ fontWeight: 700, color: "#1E2B4A", marginBottom: 2 }}>
            {project.title}
          </h5>
        ...
        <div className="d-flex gap-2 flex-shrink-0 flex-wrap justify-content-end">
          ...
          {project.is_paid && (
            ...
            >
              <FaMoneyBillWave size={10} style={{ marginRight: 4 }} />
              Paid
            </span>
          )}
        </div>
      ...
      >
        ...
        <div className="d-flex gap-2">
          ...
          {isOwner && (
            <span
              style={{ fontSize: "0.72rem", color: "#2952A3", fontWeight: 600 }}
            >
              Your Project
            </span>
          )}

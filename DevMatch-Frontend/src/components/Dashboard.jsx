...
import { API_URL } from "../config";
  FaRocket, FaSearch, FaComments, FaUserEdit, FaPlus,
  FaFolderOpen, FaClock, FaBell, FaFire,
} from 'react-icons/fa';
...
function StatCard({ icon, value, label, color, path, navigate, highlight, onClick }) {
  const handleClick = () => {
    if (onClick) onClick();
  ...
  return (
    <div
      onClick={handleClick}
      style={{
        background: highlight ? `linear-gradient(135deg, ${color} 0%, ${color}cc 100%)` : '#fff',
        borderRadius: 14,
        padding: '20px',
        boxShadow: highlight
          ? `0 4px 20px ${color}40`
...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (!user) return;
    fetch(`${API_URL}/auth/stats`, { credentials: 'include' })
  ...
  const statCards = [
    { icon: <FaFolderOpen />, value: stats?.projects_owned,      label: 'Projects Created',  color: '#2952A3', path: '/owner/projects',           highlight: true },
    { icon: <FaRocket />,     value: stats?.projects_joined,     label: 'Projects Joined',   color: '#4338CA', path: '/owner/projects?tab=joined' },
    { icon: <FaClock />,      value: stats?.pending_requests,    label: 'Pending Requests',  color: '#D97706', path: '/owner/projects?tab=requests' },
    { icon: <FaComments />,   value: stats?.active_chats,        label: 'Active Chats',      color: '#059669', path: '/chats' },
    { icon: <FaBell />,       value: stats?.unread_notifications, label: 'Notifications',    color: '#0EA5E9',
      onClick: () => window.dispatchEvent(new Event('devmatch:open-notifications')) },
  ];

  const quickActions = [
    { icon: <FaSearch />,   label: 'Browse Projects', desc: 'Find projects to join',         path: '/projects',      color: '#4B6FCE' },
    { icon: <FaFire />,     label: 'Find Matches',    desc: 'Projects that fit your skills', path: '/matches',       color: '#2952A3' },
    { icon: <FaPlus />,     label: 'Create Project',  desc: 'Start a new project',           path: '/create-project', color: '#0EA5E9' },
    { icon: <FaComments />, label: 'My Chats',        desc: 'View team conversations',       path: '/chats',         color: '#10B981' },
    { icon: <FaUserEdit />, label: 'Edit Profile',    desc: 'Update your info & skills',     path: '/edit-profile',  color: '#F59E0B' },
  ];
      ...
      <div style={{ maxWidth: 900, margin: '0 auto' }}>
        ...
        <div style={{
          ...
          <div style={{ flex: 1 }}>
            ...
            <p style={{ color: '#8A9BBC', marginBottom: 0, fontSize: '0.9rem' }}>
          ...
          {!user.is_profile_completed && (
            <button
              className="btn"
              style={{ background: '#2952A3', color: '#fff', border: 'none', borderRadius: 10, fontWeight: 600, padding: '10px 20px', flexShrink: 0 }}
        ...
        {/* Stats row */}
        <h5 style={{ fontWeight: 700, color: '#1E2B4A', marginBottom: 14, paddingLeft: 2 }}>
          Your Activity
        </h5>
        <div className="row g-3 mb-4">
          {statCards.map((s) => (
            <div key={s.label} className="col-6 col-md-4 col-lg">
              {statsLoading ? (
                <div style={{ background: '#fff', borderRadius: 14, padding: 20, height: 110, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <div key={action.path} className="col-6 col-md-4 col-lg">
              ...
              >
                <div style={{
                  ...
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  fontSize: '1.1rem', margin: '0 auto 10px',
                }}>
                  {action.icon}
                </div>
                <div style={{ fontWeight: 600, fontSize: '0.85rem', color: '#1E2B4A', marginBottom: 3 }}>
                  {action.label}
                </div>

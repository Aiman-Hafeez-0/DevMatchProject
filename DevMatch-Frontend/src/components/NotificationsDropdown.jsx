...
export default function NotificationsDropdown({ setNotifCount }) {
  ...
  const fetchNotifications = async () => {
    try {
      const res = await fetch(`${API_URL}/projects/notifications`, {
        credentials: "include",
      });
      if (!res.ok) return;
  ...
  const dismissNotification = async (notifId, e) => {
    ...
    try {
      await fetch(`${API_URL}/projects/notifications/${notifId}`, {
        method: "DELETE",
        credentials: "include",
      });
    } catch (err) {
  ...
  const clearAll = async () => {
    ...
    setNotifCount(0);
    try {
      await fetch(`${API_URL}/projects/notifications/clear`, {
        method: "DELETE",
        credentials: "include",
      });
    } catch (err) {
  ...
  const handleAccept = async (e, notifId, project_id, participant_id) => {
    ...
    try {
      const res = await fetch(
        `${API_URL}/projects/${project_id}/accept/${participant_id}`,
  ...
  const handleReject = async (e, notifId, project_id, participant_id) => {
    ...
    try {
      const res = await fetch(
        `${API_URL}/projects/${project_id}/reject/${participant_id}`,
  ...
  const handleAcceptInvite = async (e, notifId, project_id) => {
    ...
    try {
      const res = await fetch(
        `${API_URL}/projects/${project_id}/invite/accept`,
        { method: "PUT", credentials: "include" }
      );
      if (res.ok) {
  ...
  const handleDeclineInvite = async (e, notifId, project_id) => {
    ...
    try {
      const res = await fetch(
        `${API_URL}/projects/${project_id}/invite/decline`,
        { method: "DELETE", credentials: "include" }
      );
      if (res.ok) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        padding: "14px 16px", borderBottom: "1px solid #EEF2FB", flexShrink: 0,
      }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <FaBell size={14} color="#2952A3" />
          <span style={{ fontWeight: 700, fontSize: "0.92rem", color: "#1E2B4A" }}>Notifications</span>
        </div>
        {notifications.length > 0 && (
          <button
            ...
            style={{
              ...
              fontSize: "0.75rem", color: "#8A9BBC", fontWeight: 500,
      ...
      <div style={{ overflowY: "auto", flex: 1 }}>
        ...
        {!loading && notifications.length === 0 && (
          <div style={{ padding: "40px 20px", textAlign: "center" }}>
            ...
            <p style={{ color: "#A6B3CF", fontSize: "0.85rem", margin: 0 }}>You're all caught up!</p>
          </div>
        )}
        ...
        {notifications.map((n) => {
          ...
          const meta = TYPE_META[n.type] || { icon: <FaBell />, color: "#8A9BBC" };
          const projectId = parsedData.project_id;
          const isClickable = !!projectId && !["join_request", "project_invite"].includes(n.type);
          ...
          return (
            <div
              ...
              onMouseEnter={(e) => { if (isClickable) e.currentTarget.style.background = "#F7F9FD"; }}
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div className="d-flex justify-content-between align-items-start gap-2">
                  <p style={{ fontSize: "0.83rem", color: "#1E2B4A", margin: 0, lineHeight: 1.45 }}>
                    {n.message}
                  </p>
                  <button
                    type="button"
                    ...
                    style={{
                      background: "none", border: "none", cursor: "pointer",
                      color: "#C7D0E5", fontSize: "0.95rem", lineHeight: 1,
                      padding: 0, flexShrink: 0,
                    }}
                    onMouseEnter={(e) => (e.currentTarget.style.color = "#DC2626")}
                    onMouseLeave={(e) => (e.currentTarget.style.color = "#C7D0E5")}
                  >
                    ×
                  </button>
                </div>
                ...
                <div className="d-flex align-items-center justify-content-between mt-1">
                  <span style={{ fontSize: "0.68rem", color: "#A6B3CF" }}>{formatTime(n.created_at)}</span>
                  {isClickable && (
                    <span style={{ fontSize: "0.68rem", color: meta.color, display: "flex", alignItems: "center", gap: 2, fontWeight: 600 }}>
                ...
                {n.type === "join_request" && (
                  <div className="d-flex gap-2 mt-2">
                    <button
                      ...
                      onClick={(e) => handleAccept(e, n.id, parsedData.project_id, parsedData.participant_id)}
                      style={{ background: "#16a34a", color: "#fff", border: "none", borderRadius: 6, padding: "4px 12px", fontSize: "0.75rem", fontWeight: 600, cursor: "pointer" }}
                    >
                      {processingId === n.id ? "..." : "Accept"}
                    </button>
                    <button
                      disabled={processingId === n.id}
                      onClick={(e) => handleReject(e, n.id, parsedData.project_id, parsedData.participant_id)}
                      style={{ background: "#fff", color: "#DC2626", border: "1px solid #FECACA", borderRadius: 6, padding: "4px 12px", fontSize: "0.75rem", fontWeight: 600, cursor: "pointer" }}
                    >
                ...
                {n.type === "project_invite" && (
                  <div className="d-flex gap-2 mt-2">
                    <button
                      disabled={processingId === n.id}
                      onClick={(e) => handleAcceptInvite(e, n.id, parsedData.project_id)}
                      style={{ background: "#16a34a", color: "#fff", border: "none", borderRadius: 6, padding: "4px 12px", fontSize: "0.75rem", fontWeight: 600, cursor: "pointer" }}
                    >
                      {processingId === n.id ? "..." : "Accept"}
                    </button>
                    <button
                      disabled={processingId === n.id}
                      onClick={(e) => handleDeclineInvite(e, n.id, parsedData.project_id)}
                      style={{ background: "#fff", color: "#DC2626", border: "1px solid #FECACA", borderRadius: 6, padding: "4px 12px", fontSize: "0.75rem", fontWeight: 600, cursor: "pointer" }}
                    >

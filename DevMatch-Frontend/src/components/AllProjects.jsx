...
const URGENCY_PILL = {
  High: {
    active: { background: "#e53e3e", color: "#fff" },
    idle: {
      background: "#FEF2F2",
      color: "#e53e3e",
  ...
  Moderate: {
    active: { background: "#d97706", color: "#fff" },
    idle: {
      background: "#fffbeb",
      color: "#d97706",
...
export default function AllProjects() {
  ...
  useEffect(() => {
    fetch("http://localhost:5000/projects/all", { credentials: "include" })
      .then((r) => {
  ...
  const filtered = useMemo(() => {
    ...
    return projects.filter((p) => {
      ...
      if (
        ...
        !ts.some((t) => t.toLowerCase().includes(q))
      )
        return false;
      ...
      if (
        techFilter &&
        !ts.map((t) => t.toLowerCase()).includes(techFilter.toLowerCase())
      )
        return false;
      return true;
    });
    ...
    <div style={BG}>
      <div style={{ maxWidth: 960, margin: "0 auto" }}>
        ...
        <div className="d-flex justify-content-between align-items-end mb-4 flex-wrap gap-3">
          <div>
            ...
            <p style={{ color: "#8A9BBC", marginBottom: 0 }}>
              <span style={{ fontWeight: 700, color: "#2952A3" }}>
                {filtered.length}
              </span>{" "}
        ...
        >
          ...
          {allTechs.length > 0 && (
            <div
              className="d-flex flex-wrap gap-1 mt-3 pt-3"
            ...
            >
              ...
              {allTechs.map((t) => {
                ...
                return (
                  <button
                    key={t}
                    onClick={() => setTechFilter(isActive ? "" : t)}
                    style={{
        ...
        {!error && filtered.length === 0 && (
          ...
          >
            ...
            <p style={{ color: "#8A9BBC", marginBottom: 12 }}>
        ...
        <div
          className={
            viewMode === "grid" ? "row g-3" : "d-flex flex-column gap-3"
          }
        >
          {filtered.map((project) => (
            <div
              key={project.id}
              className={viewMode === "grid" ? "col-12 col-md-6" : undefined}
            >
